...
class UrlReceiveActivity : AppCompatActivity(), CoroutineScope {
    val CREATE_FILE = 1002
    var pdf = byteArrayOf()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val orig_url_n = when (intent.action) {
            Intent.ACTION_SEND -> {
        ...
        val orig_url = orig_url_n ?: run {
            logcat.append("error: Could not get URL from Intent\n")
            return
        }

        logcat.append("Received url: " + orig_url + "\n")
        getUrls(orig_url)
    }
